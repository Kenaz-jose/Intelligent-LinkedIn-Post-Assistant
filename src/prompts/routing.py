...
ROUTING RULES:
1. If Faithfulness Pass is False or the critique mentions unsupported/invented claims, you MUST choose 'fix_facts'. (This overrides everything else).
2. If facts are solid but the hook is weak, generic, or boring, choose 'fix_hook'.
3. If facts and hook are good, but the transitions, formatting, or tone are lacking, choose 'fix_flow'.
4. If the draft meets standards or if further edits will yield diminishing returns, choose 'finalize'.
...
# Pre-compile the template so the agent can just import it
